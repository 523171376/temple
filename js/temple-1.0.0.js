...
var Temple = function(){
	...
	this.listPut = function(str){
		var targetStr = '';
		//FIXME 添加 ${#if: length eq '3'}  ${#if/} 标签嵌套赋值问题
//		targetStr = _temple.replaceAll(str, /\$\{#if\:([\W\w](?!)[\W\w])*#if\/\}/g);
		//单行匹配
		targetStr = _temple.replaceAll(str, /\$\{#list\:.*(.(?!\$\{#list\/\}).)*#list\/\}/g);
		//多行匹配  FIXME 截断词后回车换行匹配截断失效问题
...
//			xhr.open('GET', _temple.templeURL + zurl + '.ht', false);
//			xhr.onreadystatechange = function(){
//				if(xhr.readyState === 4 && xhr.status === 200){
//					res = xhr.responseText;
//			    	if(data){
//			    		_temple.cacheData = data;
//			    		res = _temple.putDatas(xhr.responseText);
//			    	}
//				}
//			}
//			xhr.send();
